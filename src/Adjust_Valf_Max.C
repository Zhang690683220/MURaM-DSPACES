...
#pragma acc parallel loop collapse(3)                              \
 ...
 present(Grid[:1], Grid.U[:bufsize], Grid.pres[:bufsize])
  ...
  if(Physics.params[i_param_va_adjust] > 0.0){
    ...
    if(ini_flag){
      v_lim = min(g_max[0],Physics.tchk[i_tchk_vmax]);
      e_lim = min(g_max[2],Physics.tchk[i_tchk_eps_max]);
    ...
    va_max = max(Physics.params[i_param_va_max],
	Physics.params[i_param_va_adjust]*vv_fac*g_max[0]);
    va_max = max(va_max,Physics.params[i_param_va_adjust]*cs_fac*g_max[1]);

    va_max = max(va_max,va_max_old*eps_decay);
    
    v_lim = max(v_lim,Physics.params[i_param_va_max]/vv_fac);
    v_lim = min(v_lim,Physics.tchk[i_tchk_vmax]);

    e_lim = max(e_lim,pow(Physics.params[i_param_va_max]/cs_fac,2));
    e_lim = min(e_lim,Physics.tchk[i_tchk_eps_max]);
  }  else {
