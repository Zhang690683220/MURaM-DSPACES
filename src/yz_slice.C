...
void yz_slice(const RunData&  Run, const GridData& Grid, 
	...
	if(ini_flag ==1) {
		...
		if(Run.rank == 0) {
			if(Run.verbose > 1) {
				cout << "yz_slice: " << nslice << endl;
			}
			for (i=0;i<nslice;i++){
				if(Run.verbose > 1) {
					cout << "yz_slice: " << ixpos[i]<< endl;
				}
			}     
		}
		...
		log_entry_init(io_file_log->yz, (char*)"YZ", est_total_slice_iters);
		if(Run.use_dspaces_io) {
			io_dspaces_log->yz = (struct log_entry*) malloc(sizeof(struct log_entry));
			log_entry_init(io_dspaces_log->yz, (char*)"YZ", est_total_slice_iters);
		}
	}
    	...
    	if((Grid.beg[0] <= ixpos[nsl]+Grid.gbeg[0]) and 
			...
			if(Run.use_dspaces_io) {
				...
				if(ini_flag) {
					...
					char vname[128];
					for(int idx=0; idx<nslvar; i++) {
						sprintf(vname, "%s_%d", ds_var_name, idx);
						dspaces_define_gdim(ds_client, vname, 2, gdim);
					}
				}
				clk = MPI_Wtime();
				slice_write_dspaces(Grid, 0, iobuf, localsize, nslvar, 1, 2, ds_var_name,
									Run.globiter, 2);
	...
	if(Run.rank == 0 && Run.verbose >0) {
		...
		if(Run.use_dspaces_io) {
			...
			io_dspaces_log->yz->wait_time[io_dspaces_log->yz->count] = 0.0;
			io_dspaces_log->yz->time[io_dspaces_log->yz->count] = dspaces_time;
