...
// frequency dependent quantities
  ...
  del_d3dim(W_0_array,yl,yh,xl,xh,zl,zh);
...
//*/
  del_f1dim(tab_T,0,NT-1);
  del_f1dim(tab_p,0,Np-1);
  del_f1dim(invT_tab,0,NT-1);
  del_f1dim(invP_tab,0,Np-1);
...
//
  ...
  for(int v=0;v<3;v++){
    ...
    lrank[v] = Grid.lrank[v];
    cart_sizes[v] = Grid.procs[v];
    cout << v << " " << Grid.stride[v] << " " << Grid.lrank[v] << " " << Grid.procs[v] << endl;
  }
 
  for(int nleft,nright,nd=0;nd<3;nd++){
...
// frequency dependent quantities
  ...
  W_0_array=d3dim(yl,yh,xl,xh,zl,zh);
...
void RTS::load_bins(){
  ...
  ifstream fp_rt("./kappa.dat", ios::in | ios::binary);

  /* DP:
  Junk1, Junk2 and Junk3 are spare header variables, it is
  expected these will be needed for future bin setups. IE.
  scatter on/off, ability to switch between stagger/MURaM bin
  styles, and a new kap file to provide an optical depth scale
  needed for coronal back-heating */
  ...
  if (fp_rt.is_open()) {
    ...
    invT_tab = f1dim(0,NT-1);
    invP_tab = f1dim(0,Np-1);
    
    fp_rt.read((char*)&tab_T[0],NT*sizeof(float));
    fp_rt.read((char*)&tab_p[0],Np*sizeof(float));
    ...
    cout << "rt_init: kappa file not found" << endl;
    MPI_Abort(MPI_COMM_WORLD,1);
  }
...
void RTS::calc_Qtot_and_Tau(GridData &Grid, const RunData &Run, const PhysicsData &Physics){
 ...
 double tau_min = Physics.rt[i_rt_tau_min];
  ...
  for(int y=yl+yo;y<=yh;++y) // loop over MHD grid
    for(int x=xl+xo;x<=xh;++x){
      ...
      for(int z=zl+zo;z<=zh;++z){
        ...
        double scale = Grid.Tau[node]*Grid.Tau[node];
        scale = scale/(scale + tau_min*tau_min);
...
void RTS::driver(int ZMAX,int XMAX,int YMAX, double DZ, double DX, double DY, int band){
  ...
  memset(W_0_array[yl][xl]+zl,0,nx*ny*nz*sizeof(double));
...
// read BC from recvbuf
          stime = MPI_Wtime();
          readbuf(ZMAX,XMAX,YMAX,band,l,ZDIR,XDIR,YDIR);
...
// loop over the grid points
          ...
          stime = MPI_Wtime();
          writebuf(ZMAX,XMAX,YMAX,band,l,ZDIR,XDIR,YDIR); 
          buf_time += MPI_Wtime()-stime;
          stime = MPI_Wtime();
          exchange(ZMAX, XMAX, YMAX, band, l, ZDIR, XDIR,YDIR);    
          etime += MPI_Wtime()-stime;
          
          if(rt_iter>=rt_min_iter){
            stime = MPI_Wtime();
            double err=error(ZMAX,XMAX,YMAX,band,l,ZDIR,XDIR,YDIR,I_min);
...
// increment for F and J 
            stime=MPI_Wtime();
            flux(ZMAX,XMAX,YMAX,l,ZDIR,XDIR,YDIR);
...
// continuum band last -> can use tau in tau_slice as for grey RT 
  double stime=MPI_Wtime();
  tauscale_qrad(ZMAX,XMAX,YMAX,band,DX,DY,DZ,B);
           ...
           int yi_i,int yi_f,int ystep,int l,double** coeff, double *** Ss)
{
    ...
    for(int yi=yi_i;yi!=yi_f+ystep;yi=yi+ystep){
      for(int xi=xi_i;xi!=xi_f+xstep;xi=xi+xstep){
        ...
        for(int zi=zmin;zi<=zmax;++zi){
         ...
         if (dt[zi] > dtau_min)
            W_0_array[yi][xi][zi] += 1.0-w0;
          else
            W_0_array[yi][xi][zi] += 0.5*dt[zi];
        }   
  ...
  if(NDIM==1){
    ...
    for(int zi=zmin;zi<=zmax;++zi){
      ...
      if (dt[zi] > dtau_min)
        W_0_array[0][0][zi] += 1.0-w0;
      else
        W_0_array[0][0][zi] += 0.5*dt[zi];
    }
  ...
  if(NDIM==2){
    for(int xi=xi_i;xi!=xi_f+xstep;xi=xi+xstep){
      ...
      for(int zi=zmin;zi<=zmax;++zi){
        ...
        if (dt[zi] > dtau_min)
          W_0_array[0][xi][zi] += 1.0-w0;
        else
          W_0_array[0][xi][zi] += 0.5*dt[zi];
     }
...
double RTS::error(int ZMAX,int XMAX,int YMAX,int band,int l,int ZDIR,int XDIR,int YDIR,double I_min)
{
...
void RTS::readbuf(int ZMAX,int XMAX,int YMAX,int band,int l,int ZDIR,int XDIR,int YDIR)
{//RHC
  if(NDIM==3){
...
void RTS::writebuf(int ZMAX,int XMAX,int YMAX,int band, int l,int ZDIR,int XDIR,int YDIR)
{
  if (NDIM==3){
...
void RTS::exchange(int ZMAX,int XMAX,int YMAX,int band,int l,int ZDIR,int XDIR,int YDIR)
{
...
void RTS::flux(int ZMAX,int XMAX,int YMAX,int l,int ZDIR,int XDIR,int YDIR)
{
...
void RTS::get_Tau_and_Iout(GridData &Grid, const RunData &Run, const PhysicsData &Physics, double DZ, float * B_Iout_tab, float ** kap_Iout_tab, double ** I_band, int calc_int){
  ...
  for(int y=yl;y<=yh;++y){ // loop over RT grid
    ...
    //disbale RT if tm > 2e4 above the photosphere
    ...
    tr_switch[y][x][z] = (int) max(pswitch,tswitch);
        ...
        kap[y][x][z] =
        exp(xt*(xp*kap_Iout_tab[l+1][m+1]+(1.-xp)*kap_Iout_tab[l+1][m])+
  ...
  MPI_Scan(sbuf[yl]+xl,rbuf[yl]+xl,nx*ny,MPI_DOUBLE,MPI_SUM,XCOL_COMM);
  ...
  ctime=MPI_Wtime(); 
  MPI_Allreduce(sbuf[yl]+xl,rbuf[yl]+xl,nx*ny,MPI_DOUBLE,MPI_SUM,XCOL_COMM);
...
void RTS::tauscale_qrad(int ZMAX,int XMAX,int YMAX,int band, double DX,double DY,double DZ, double *** Ss){
