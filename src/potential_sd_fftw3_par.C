...
void potential_ext_fftw3_par(const GridData& Grid, double *bz0, double *b_ext){
  ...
  if(ini_flag){
	...
	if(x_rank == 0){
	  ...
	  plan_fwd = fftw_mpi_plan_dft_2d(N0, N1, data_in, data_out, y_comm, FFTW_FORWARD,  FFTW_PATIENT);
	  plan_bwd = fftw_mpi_plan_dft_2d(N0, N1, data_in, data_out, y_comm, FFTW_BACKWARD, FFTW_PATIENT);
