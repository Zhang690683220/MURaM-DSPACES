...
void OutputSolution(const RunData& Run,const GridData& Grid,const PhysicsData& Physics)
{
	...
	if(Physics.params[i_param_spitzer] > 0.0) {
  	...
  	for(v2=0;v2<v2_max;v2++) {
    	if(zcol_rank == v2) {
			iobuf_glo = (float*)malloc(gsize*sizeof(float));
		}
	}
    	...
    	for(v2=0;v2<v2_max;v2++) {
      		...
      		if(zcol_rank == v2) {
				...
				if(mpi_io_out == 0) {
	  				...
	  				for(k=0;k<nblocks;k++) {
						xy_slice_write(Grid,0,&(iobuf_glo[k*blsz*sizex*sizey]),sizex*sizey,fh);
					}
	  				if(xy_rank == 0) {
						fclose(fh);
					}
		...
		for(v2=0;v2<v2_max;v2++) {
			...
			if(zcol_rank == v2) {
				...
				if(mpi_io_in == 0) {
					...
					for(k=0;k<nblocks;k++) {
						xy_slice_read(Grid,0,&(iobuf_glo[k*blsz*sizex*sizey]),sizex*sizey, fh);
					}
					if(xy_rank == 0) {
						fclose(fh);
					}
					...
					MPI_File_set_view(mfh,0,MPI_FLOAT,io_subarray,(char *) "native", io_info);
					MPI_File_read_all(mfh,iobuf_glo,gsize,MPI_FLOAT,MPI_STATUS_IGNORE);
					MPI_File_close(&mfh);
				}
			}
		}
...
void diag_output(const RunData& Run, const GridData& Grid,const PhysicsData& Physics, RTS *rts) {
	...
	if(ini_flag == 1) {
		...
		log_entry_init(io_file_log->diag, (char*)"DIAG", est_total_res_iters);

		if(Run.use_dspaces_io) {
			io_dspaces_log->diag = (struct log_entry*) malloc(sizeof(struct log_entry));
			log_entry_init(io_dspaces_log->diag, (char*)"DIAG", est_total_res_iters);
		}
	}
	...
	if(Run.use_dspaces_io) {
		...
		for(int vi=0; vi<v_max; vi++) {
			...
			clk = MPI_Wtime();
			dspaces_put(ds_client, ds_var_name, Run.globiter,
		...
		if(io_rank == 0) {
			...
			io_dspaces_log->diag->wait_time[io_dspaces_log->diag->count] = 0.0;
...
void eos_output(const RunData& Run, const GridData& Grid,const PhysicsData& Physics, RTS *rts) {
	...
	if(ini_flag == 1) {
		...
		log_entry_init(io_file_log->eos, (char*)"EOS", est_total_res_iters);

		if(Run.use_dspaces_io) {
			io_dspaces_log->eos = (struct log_entry*) malloc(sizeof(struct log_entry));
			log_entry_init(io_dspaces_log->eos, (char*)"EOS", est_total_res_iters);
		}
	}
	...
	if(Run.use_dspaces_io) {
		...
		if(io_rank == 0 && Run.verbose > 0) {
			...
			io_dspaces_log->eos->wait_time[io_dspaces_log->eos->count] = 0.0;
