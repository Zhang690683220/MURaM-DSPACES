...
void AnalyzeSolution_VP(const RunData& Run, const GridData& Grid,
	...
	if(ini_flag) {
		...
		log_entry_init(io_file_log->analyze_vp, (char*)"ANALYZE_VP", est_total_slice_iters);
		if(Run.use_dspaces_io) {
			io_dspaces_log->analyze_vp = (struct log_entry*) malloc(sizeof(struct log_entry));
			log_entry_init(io_dspaces_log->analyze_vp, (char*)"ANALYZE_VP", est_total_slice_iters);
		}
	}
	...
	for(j=0;j<bufsz;j++) { 
  	...
  	LOCAL_LOOP(Grid,i,j,k) {
		...
		if(Physics.rt_ext[i_ext_cor] == 1) {
			...
			rfac = (Grid.temp[node] > Physics.rt[i_rt_tr_tem]) &&
					(Grid.pres[node] < Physics.rt[i_rt_tr_pre]);
		} else {
			rfac = 0.0;
		}
  	...
  	if(yz_rank==0){ // MPI_Reduce results are only meaningful on rank 0!
		...
		if(Run.use_dspaces_io) {
			...
			for(int v=0; v<nvar; v++) {
				...
				clk = MPI_Wtime();
				dspaces_put(ds_client, ds_var_name, Run.globiter, sizeof(float),
	...
	if(yz_rank == 0 && xcol_rank == 0) {
		...
		if(Run.use_dspaces_io) {
			...
			io_dspaces_log->analyze_vp->wait_time[io_dspaces_log->analyze_vp->count] = 0.0;
