...
void xz_slice(const RunData&  Run, const GridData& Grid, 
	...
	if(ini_flag ==1) {
		...
		if(Run.rank == 0) {
			if(Run.verbose > 1) {
				cout << "xz_slice: " << nslice << endl;
			}
			for(i=0;i<nslice;i++) {
				if(Run.verbose > 1) {
					cout << "xz_slice: " << ixpos[i]<< endl;
				}
			}     
		}
		...
		log_entry_init(io_file_log->xz, (char*)"XZ", est_total_slice_iters);
		if(Run.use_dspaces_io) {
			io_dspaces_log->xz = (struct log_entry*) malloc(sizeof(struct log_entry));
			log_entry_init(io_dspaces_log->xz, (char*)"XZ", est_total_slice_iters);
		}
	}
  	...
  	for(nsl = 0; nsl<nslice; nsl++) {
    	if((Grid.beg[1] <= ixpos[nsl]+Grid.gbeg[1]) and 
			...
			if(Run.use_dspaces_io) {
				...
				clk = MPI_Wtime();
				slice_write_dspaces(Grid, 0, iobuf, localsize, nslvar, 2, 0, ds_var_name,
									Run.globiter, 2);
	...
	if(Run.rank == 0) {
		...
		if(Run.use_dspaces_io) {
			...
			io_dspaces_log->xz->wait_time[io_dspaces_log->xz->count] = 0.0;
