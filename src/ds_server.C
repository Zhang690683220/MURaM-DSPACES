...
void write_eos(dspaces_client_t client, const RunData& Run, const GridData& Grid,const PhysicsData& Physics, int globiter, MPI_Comm comm)
{
    ...
    const char*envdecomp = getenv("DSPACES_DECOMP3D");
    if(envdecomp) {
...
void nc_write_eos(dspaces_client_t client, const RunData& Run, const GridData& Grid,
                    const PhysicsData& Physics, int globiter, MPI_Comm comm)
{
    ...
    const char* envdecomp3d = getenv("DSPACES_DECOMP3D");
    if(envdecomp3d) {
        decomp3d = atoi(envdecomp3d);
    }
    ...
    nc_ret = nc_create_par(filename, NC_CLOBBER | NC_NETCDF4, comm, MPI_INFO_NULL, &nc_fid);
    if(nc_ret != NC_NOERR) {
    ...
    for(int d=0; d<3; d++) {
        nc_ret = nc_def_dim(nc_fid, nc_dimname[d], gsz[d], &nc_dimid[d]);
        if(nc_ret != NC_NOERR) {
    ...
    for(int v=0; v<tot_vars; v++) {
        ...
        if(f_gdim) {
            dspaces_define_gdim(client, ds_var_name, 3, gsz);
        }
    ...
    for(int nsl=0; nsl<nslice; nsl++) {
        if(Physics.slice[i_sl_collect] == 0) {
            ...
            for(int v=0; v<nslvar; v++) {
                ...
                if(f_gdim) {
                    dspaces_define_gdim(client, ds_var_name, 2, gsz);
                }
