...
void tau_slice(const RunData&  Run, const GridData& Grid, 
	...
	if(Run.use_dspaces_io) {
  	...
  	if(ini_flag ==1) {
		...
		log_entry_init(io_file_log->tau, (char*)"TAU", est_total_slice_iters);
		if(Run.use_dspaces_io) {
			io_dspaces_log->tau = (struct log_entry*) malloc(sizeof(struct log_entry));
			log_entry_init(io_dspaces_log->tau, (char*)"TAU", est_total_slice_iters);
		}
  	}
        		...
        		for(i=ibeg; i<=iend; i++) {
					...
					node2 = Grid.node(i+1,j,k);
	  				...
	  				if((Grid.Tau[node1] >= tau_lev[nsl]) && (tau_lev[nsl] > Grid.Tau[node2])) {
	    				...
	    				// q1 = (log(tau_lev[nsl])-log(Grid.Tau[node2]))/(log(Grid.Tau[node1])-
						// 		log(Grid.Tau[node2]));
	    				q2 = 1.0-q1;
						...
						if(Physics.tau_var[4] == 1) {
							iobuf[ind1] = (float) (Grid.U[node1].e/Grid.U[node1].d*q1+Grid.U[node2].e/
													Grid.U[node2].d*q2);
							ind1 += localsize;
						}
						...
						if(Physics.tau_var[8] == 1) {
							iobuf[ind1] = (float) (sqrt(Grid.U[node1].M.sqr())*q1+
													sqrt(Grid.U[node2].M.sqr())*q2);
							ind1 += localsize;
						}
						if(Physics.tau_var[9] == 1) {
							iobuf[ind1] = (float) (sqrt(Grid.U[node1].B.sqr())*q1+
													sqrt(Grid.U[node2].B.sqr())*q2);
							ind1 += localsize;
						}
						...
						if(Physics.tau_var[13] == 1) {
							iobuf[ind1] = float(Grid.coord(i,0)*q1+Grid.coord(i+1,0)*q2)/
												float(Grid.gxmax[0]);
						}
	  				}
				}
		...
		if(xcol_rank == iroot) {
			...
			*/
			...
			if(Run.use_dspaces_io) {
				...
				clk = MPI_Wtime();
				slice_write_dspaces(Grid, 0, &(iosum[0]), localsize, nslvar, 1, 2,
	...
	if(xcol_rank == 0 && yz_rank == 0) {
		...
		if(Run.use_dspaces_io) {
			...
			io_dspaces_log->tau->wait_time[io_dspaces_log->tau->count] = 0.0;
